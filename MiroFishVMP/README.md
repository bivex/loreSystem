...
- auto-promotion пока намеренно узкий и audit-friendly:
  ...
  - `safe_cross_run_rumor_only` → только `rumor_candidate -> Rumor`, но уже с cross-run stability/duplicate guard
  ...
  - `safe_cross_run_rumor_only` сначала проходит все требования `safe_rumor_only`, затем требует explicit `location_id`, unresolved `truth_level` (`Unverified` / `Partially True`), хотя бы один supporting run с тем же normalized rumor `name` / `source_name` / unresolved truth bucket и отклоняет existing staged canonical `Rumor` с тем же normalized `name` / `source_name` / `location_id`
  ...
  - auto-path пишет `auto_promote_policy` / `auto_promoted`, а cross-run policy ещё и `cross_run_supporting_run_ids`, `cross_run_distinct_run_count`; event cross-run slice дополнительно пишет `event_match_participant_refs`, `event_match_outcome`, `event_match_date_bucket`, `contradiction_check`, rumor cross-run slice — `rumor_match_name`, `rumor_match_source_name`, `rumor_truth_bucket`, `duplicate_guard`, relationship cross-run slice — `contradiction_check`
...
- auto-merge тоже остаётся explicit и audit-friendly:
  ...
  - event merge допускается только при ровно одном staged canonical `Event` exact duplicate match в том же world по canonical participant set + terminal `outcome` + UTC date bucket + optional `location_id`
  ...
  - faction merge допускается только при ровно одном staged canonical `Faction` exact duplicate match в том же world по normalized `name` + `faction_type` + `alignment` + `leader_character_id` + `is_joinable`
  ...
  - character merge допускается только при ровно одном staged canonical `Character` exact duplicate match в том же world по normalized `name` + `status` + optional `parent_id` + `location_id` + `rarity` + `element` + `role`
