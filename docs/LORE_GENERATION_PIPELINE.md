...
3.  **Резервные копии (Fallbacks)**: Если LLM ломается, возвращает некорректный JSON или превышает таймаут, система автоматически генерирует дефолтные сущности на основе шаблонов (например, создает дефолтный квест *"Событие в Гавани"*), чтобы конвейер не падал и продолжал работу. Отказоустойчивый режим теперь является бескомпромиссным стандартом.
...
* **Главное правило**: База данных SQLite **НЕ** очищается перед запуском.
* **Схема работы**:
  1. Вы формулируете локальную тему (например: *"Появление теневых монстров в гавани в результате ритуала"*).
  2. Вы выбираете уровень глубины генерации (только лор или лор + квесты).
  3. Qdrant-память автоматически извлекает релевантных персонажей (например, *Mara Voss*), их болезни и прошлые события.
  4. ИИ связывает новый слух или событие с уже существующими персонажами и фактами, органично развивая вселенную.

#### Вариант А: Добавление новых слухов и событий (Быстрый Core Pass)
Используется для создания исторического фона, изменения отношений персонажей и заселения мира новыми слухами.

* **Флаги**: `--with-memory` (кампания и системы отключены)
* **Команда**:
  ```bash
  CAMEL_MEMORY_QDRANT_URL=http://localhost:6333 \
  ./venv/bin/python3 CAMEL.Bridge/run_rumor_pipeline.py \
    --tenant-id 1 --world-id 1 \
    --theme "Последствия ритуала в гавани" \
    --context "Ивен Хейл пытается помочь Маре Восс справиться с проклятием, пока стражники ищут культистов." \
    --count 1 --db-path lore_system.db --output-language ru --env-file .env --with-memory
  ```

#### Вариант Б: Добавление новых глав и квестов (Core + Narrative Pass)
Используется для расширения сюжета: добавления новых актов, глав, квестовых цепочек или развилок в существующую кампанию.

* **Флаги**: `--with-memory --with-campaign-story` (системы отключены)
* **Команда**:
  ```bash
  CAMEL_MEMORY_QDRANT_URL=http://localhost:6333 \
  ./venv/bin/python3 CAMEL.Bridge/run_rumor_pipeline.py \
    --tenant-id 1 --world-id 1 \
    --theme "Контракт теневого клинка" \
    --context "Мара Восс просит Ивена помочь уничтожить запретный амулет кузницы." \
    --count 1 --db-path lore_system.db --output-language ru --env-file .env \
    --with-memory --with-campaign-story
  ```
